...
export function getFallbackAnalyzeResponse(footprintData: any) {
...
export function getFallbackPlaygroundResponse(systemInstruction: string, fullPrompt: string) {
  ...
  const isConstraintLimit = prompt.includes("under 120 words");
  const isConstraintBullet = prompt.includes("bullet points");
